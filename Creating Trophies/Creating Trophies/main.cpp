...
int main()
{
	...
	vector<Trophy> trophies;		// Vector that will store our trophies later
...
void copyTrophy(vector<Trophy>& trophyVector)
{
	...
	if (trophyPosition != -1)
	{
		Trophy trophyToCopy = trophyVector[trophyPosition];
...
//      none
// Return Value:
//      Trophy - returns a Trophy object
// Description:
//      Prompts the user for each value the trophy needs, creates a
//		trophy object based on those values, and then returns that object
Trophy promptForTrophy()
{
	string name = promptForString(PROMPT_FOR_NAME);
	...
	Trophy newTrophy = Trophy(name, level, color);
...
int promptForInt(const string& message, int minimum, int maximum)
{
	int userInt = 0;		// Will store the user's entered integer
...
int searchForTrophy(vector<Trophy>& trophyVector, string name)
{
	...
	while (it != trophyVector.end() && it->GetName() != name)
	{
		++it;
		++position;
	}
